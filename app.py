...
# --- Unified Grammar Notes Route ---
@app.route("/view_grammar/<string:item_type>/<int:item_id>")
def view_grammar(item_type, item_id):
    """
    Unified route to view grammar notes for both lessons and stories.
    If grammar notes don't exist, they will be generated using the selected LLM.
    """
    # Validate item_type
    if item_type not in ['lesson', 'story']:
        return "Invalid item type. Must be 'lesson' or 'story'.", 400
    ...
    if not item:
        return f"{item_type.capitalize()} not found.", 404
    
    language = item.language
    if not language:
        return "Could not determine language for the item.", 500
    
    # Get the selected LLM from query parameters
    selected_llm = request.args.get('llm', '').lower()
    
    # If no specific LLM is selected or it's invalid, check for existing grammar notes
    if selected_llm not in ['gemini', 'chatgpt']:
        if hasattr(item, 'grammar_summary') and item.grammar_summary:
            # If we have existing grammar notes, check which LLM was used
            # This is a simple heuristic - in a real app, you might want to store the LLM used
            # in the database for more accurate detection
            if 'generated by chatgpt' in item.grammar_summary.lower():
                selected_llm = 'chatgpt'
            else:
                # Default to gemini for existing notes if we can't determine
                selected_llm = 'gemini'
        else:
            # Default to gemini if no LLM specified and no existing notes
            selected_llm = 'gemini'
    
    # Check if we need to generate a new summary
    if not item.grammar_summary or request.args.get('regenerate'):
        try:
            # Call the function directly and handle the response
            result = get_grammar_summary(item_type, item_id)
            
            # Handle error response (tuple with error message and status code)
            if isinstance(result, tuple) and len(result) == 2 and isinstance(result[1], int):
                error_msg, status_code = result
                app.logger.error(f"Error generating grammar summary: {error_msg}")
                return f"Error generating grammar analysis: {error_msg}", status_code
            
            # Handle successful response
            if isinstance(result, dict) and 'summary' in result:
                item.grammar_summary = result['summary']
                db.session.commit()
            else:
                raise ValueError("Unexpected response format from grammar summary generation")
            
        except Exception as e:
            db.session.rollback()
            app.logger.error(f"Error in view_grammar: {str(e)}", exc_info=True)
            return f"Error generating grammar analysis: {str(e)}", 500
    
    # Prepare the text content for display
    text_content = item.text_content if item_type == 'lesson' else item.content
    
    return render_template(
        "grammar_view.html",
        lesson=item,  # Using 'lesson' for backward compatibility with the template
        language=language,
        grammar_summary=item.grammar_summary,
        text_content=text_content,
        selected_llm=selected_llm  # Pass the selected LLM to the template
    )
...
def get_grammar_summary(item_type, item_id):
    ...
    # 2. Check if summary already exists and we're not forcing a regenerate
    if item.grammar_summary and not request.args.get('regenerate'):
    ...
    # 3. Generate summary
    app.logger.info(f"Generating new grammar summary for {item_type} {item_id} using {selected_llm.upper()}")
    cefr_level = language.level or "intermediate"
    ...
    try:
        if selected_llm == 'gemini':
            ...
            prompt_path = os.path.join('prompts', 'grammar_analysis_prompt.txt')
            try:
            ...
            # Format the prompt with the language and text content
            full_prompt = (
                f"{prompt_template}\n\n"
                f"Language: {lang_name} (CEFR Level: {cefr_level})\n"
                f"Text to analyze:\n{text_content}"
            )
            
            # Configure generation with all safety filters explicitly disabled
            generation_config = {
                'temperature': 0.3,
            ...
            # Set safety settings to block only high probability harmful content
            safety_settings = [
            ...
            system_instruction = "You are a helpful language learning assistant that provides detailed grammar explanations."
            ...
            try:
                ...
                if not response or not hasattr(response, 'candidates') or not response.candidates:
                    raise ValueError("No response candidates were returned from the model")
                
                # Check finish_reason before accessing text
                candidate = response.candidates[0]
                if hasattr(candidate, 'finish_reason') and candidate.finish_reason == 2:  # SAFETY
                    error_msg = "Content generation was blocked by safety filters. Please try again or use a different model."
                    current_app.logger.error("Gemini API safety filter triggered")
                    return {"error": error_msg}, 400
                
                if hasattr(candidate, 'finish_reason') and candidate.finish_reason != 1:  # 1 is STOP, which is success
                    error_msg = f"Content generation stopped unexpectedly. Finish reason: {candidate.finish_reason}"
                    current_app.logger.error(f"Gemini API error - {error_msg}")
                    return {"error": error_msg}, 500
                
                # If we get here but still no text, try with a simpler prompt
                if not hasattr(response, 'text') or not response.text:
                    error_msg = "No text in response from Gemini"
                ...
                # If we get here but still no text, try with a simpler prompt
                if not hasattr(response, 'text') or not response.text:
                    app.logger.warning("Initial generation returned no text, trying with simplified prompt")
                    simplified_prompt = f"Please analyze the following text for grammar and language learning purposes. Be thorough but concise.\n\n{text_content}"
                    response = model.generate_content(simplified_prompt)
                    
                    # Check the simplified response as well
                    if not hasattr(response, 'text') or not response.text:
                        raise ValueError("Failed to generate content even with simplified prompt")
                
                # Get the generated summary
                if not response or not hasattr(response, 'candidates') or not response.candidates:
                    error_msg = "No response candidates were returned from the model"
                ...
                if hasattr(response, 'text') and response.text:
                    ...
                    error_msg = "No valid text content was generated by the model"
                    app.logger.error(error_msg)
                    return {"error": error_msg}, 500
            ...
            summary_response = openai_client.chat.completions.create(
                ...
                messages=[
                    {
                        "role": "system",
                        "content": "You are a helpful language learning assistant that provides detailed grammar explanations.",
                    },
                    {"role": "user", "content": full_prompt},
                ],
                max_tokens=10000,  # Increased to 10,000 tokens
            )
            generated_summary = summary_response.choices[0].message.content.strip()

            # Return the generated summary
            summary_data = {"summary": generated_summary}
            app.logger.info(
                f"Successfully generated grammar summary for {item_type} {item_id} using {selected_llm.upper()}"
            )
        ...
        db.session.rollback()
        error_msg = f"Error generating grammar summary with {selected_llm.upper()} for {item_type} {item_id}: {e}"
        app.logger.error(error_msg)
